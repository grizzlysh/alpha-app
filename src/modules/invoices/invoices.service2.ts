...
const getBusinessParameter = async (
  ...
  const param = await tx.businessParameter.findUnique({
...
export const createInvoice = async (
  ...
  const invoice = await prisma.$transaction(async (tx) => {
    ...
    const marginPercentage = parseFloat(
      await getBusinessParameter(pharmacyId, 'MARGIN_PERCENTAGE', tx)
    )
      ...
      data.details.map(async (detail) => {
        ...
        return {
          ...
          totalAmount: new Decimal(
            (
              detail.quantityPieces * parseFloat(finalPrice.toString())
            ).toFixed(2)
          ),
          createdById: userId,
        }
    ...
    // ── Create Stock per Detail ─────────────────────
    for (const detail of createdInvoice.details) {
      // upsert stock header
      ...
      if (!existingStock) {
        ...
        stockId = newStock.id
        quantityBefore = 0
        quantityAfter = detail.quantityPieces
      } else {
        // update price only if new price is higher
        const shouldUpdatePrice = detail.finalPrice.greaterThan(
          existingStock.basePrice
        )

        quantityBefore = existingStock.totalPieces
        quantityAfter = quantityBefore + detail.quantityPieces

        await tx.stock.update({
          where: { id: existingStock.id },
          data: {
            totalPieces: quantityAfter,
            ...(shouldUpdatePrice && {
              basePrice: detail.finalPrice,
              calculatedPrice: detail.calculatedPrice,
              isManualPrice: false,
  ...
  // fetch full invoice with all relations
