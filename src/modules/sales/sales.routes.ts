...
import { createSaleSchema, updateSaleSchema, cancelSaleSchema, addPaymentSchema, updatePaymentHistorySchema, saleQuerySchema } from './sales.validation'
...
router.patch('/:sale_uuid', requirePermission(PERMISSIONS.SALES_UPDATE), validateBody(updateSaleSchema), SaleController.updateSale)
router.patch('/:sale_uuid/complete', requirePermission(PERMISSIONS.SALES_UPDATE), validateBody(createSaleSchema), SaleController.completeSale)
